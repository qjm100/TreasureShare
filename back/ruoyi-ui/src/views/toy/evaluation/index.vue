  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="80px">
      <el-form-item label="商品ID" prop="productId">
        <el-input v-model="queryParams.productId" placeholder="请输入商品ID" clearable style="width: 200px" @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item>
...
<script>
import { listEvaluation, delEvaluation } from '@/api/toy/evaluation'
  ...
  methods: {
    getList() {
      ...
      listEvaluation(this.queryParams).then(response => {
        ...
        this.loading = false
        // If the evaluations/0 endpoint doesn't return paginated results,
    ...
    handleDelete(row) {
      ...
      const msg = row.id ? '是否确认删除该评价？' : '是否确认删除所选评价？'
      this.$modal.confirm(msg).then(function() {
        return delEvaluation(ids.join(','))
      }).then(() => {
        this.getList()
